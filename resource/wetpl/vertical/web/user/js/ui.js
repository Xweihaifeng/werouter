...
function FileProgress(file, targetID) {
  ...
  if (!this.fileProgressWrapper.length) {
    ...
    this.fileProgressWrapper = $('<div class=""></div>');
    ...
    // progressText.append("<input type='hidden' class='filesname' name='thumb_image[]' value='"+file.name+"' />");
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
  ...
  var progressBarChunkTr = $(
    '<div class="chunk-status-tr"><div colspan=3></div></div>');
...
FileProgress.prototype.setComplete = function(up, info) {
  ...
  var url;
  this.fileProgressWrapper.append("<input type='hidden' class='filesname' name='thumb_image[]' value='" +  encodeURI(res.key) + "'/>");
  if (res.url) {
    url = res.url;
    ...
    str = "<strong>Link:</strong><a href=" + url+ " target='_blank' > " +
      link + "</a></div>" +
      "<div class=hash><strong>Hash:</strong>" + res.hash + "</div>";
  }
  ...
  var imgWrapper = $('<div class="imgWrapper col-md-12"/>');
  var linkWrapper = $('<a class="linkWrapper" target="_blank"/>');
  var showImg = $('<img src="/common/img/loading.gif"/>');
  ...
  if (!isImg) {
    ...
    $(img).on('load', function() {
      ...
      function initImg(url, key, height) {
        ...
        modalBody.find('img').attr('src', '/common/img/loading.gif');
