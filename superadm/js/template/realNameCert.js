 var cloneObj = function (obj) {
    ...
    var initAdvList = function(data){
        ...
        $('#listTable').DataTable({
            "ajax": {
                url:ApiUrl+"cert/realname/lists?type=2&is_done=1",
                type:"GET",
                data:data,
                "dataSrc": function ( json ) {
                    ...
                    for ( var i=0, ien=json.data.list.length ; i<ien ; i++ ) {
                        ...
                        list[i].operation=`<div class="btn-group" role="group"><button class="btn btn-primary pop-modal" data-id="` + json.data.list[i].weid + `">审批</button>`;
                    }
    ...
    $(document).on('click', '.pop-modal', function(){
        $('textarea[name="refuse_words"]').val('');
        ...
        $.ajax({
            ...
            success: function(data){
               ...
               if(data.data.avatar!=null && data.data.avatar!=''){
                $('#avatar').show();
                $('#avatar').attr('src', data.data.avatar.indexOf('http') === 0 ? data.data.avatar : ApiMaterPlatQiniuDomain + data.data.avatar);
               }else{
                $('#avatar').hide();
               }
                $('#real_name').text(data.data.name);
                ...
                if (data.data.operation_status == 3) {
                    $('#refuse_view').show();
                    $('#refuse_words').text(data.data.refuse_words);
                }
                $('#created_at').text(new Date(data.data.created_at * 1000).format('yyyy-MM-dd hh:mm:ss'));
            },
            error: function(xhr){
                console.log(xhr);
            }
    ...
    $(document).on('click', 'input[name=result]', function(){
        if ($('input[name="result"]:checked').val() != '' && $('.sub-btn').length === 0) {
            $('.btn-cancel').before('<button type="button" class="btn btn-success sub-btn" data-dismiss="modal">提交审核</button>');
        }
        if ($('input[name="result"]:checked').val() == 2) {
            $('#refuse_control').show();
        }
        else if ($('input[name="result"]:checked').val() == 1) {
            $('#refuse_control').hide();
        }
    });
    ...
    $(document).on('click', '.sub-btn', function(){
        ...
        var refuse_words = $('#bootstrapModal textarea[name=refuse_words]').val();
        $.ajax({
